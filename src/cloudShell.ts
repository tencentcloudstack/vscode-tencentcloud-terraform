...
export class TencentCloudShell extends BaseShell {
    ...
    public async runTerraformCmd(tfCommand: string): Promise<void> {
        vscode.window.showWarningMessage("The cloud shell is unimplemented.", tfCommand);
        return;
    }

    public async runNormalCmd(tfCommand: string): Promise<void> {
        vscode.window.showWarningMessage("The cloud shell is unimplemented.", tfCommand);
        return;
    }
    ...
    protected async runTFCommand(command: string, workdir: string): Promise<boolean> {
        if (this.terminal) {
            ...
            if (this.isCombinedWithPush(command)) {
                await vscode.commands.executeCommand("tcTerraform.push");
            }
